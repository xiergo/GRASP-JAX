...
usage() {
        ...
        echo "-z <feature_pickle>"
        echo "-q <restraints_pickle>"
        echo "-y <mrc_path>       Path to a MRC file containing density map"
        echo "-s <resolution>     Resolution of the density map"
        echo "-x <name>          Name of the protein"
...
while getopts ":d:o:f:t:z:q:g:r:e:n:a:m:c:p:l:b:y:s:x" i; do
        case "${i}" in
        d)
                data_dir=$OPTARG
        ;;
        ...
        ;;
        z)
                feature_pickle=$OPTARG
        ;;
        q)
                restraints_pickle=$OPTARG
        ;;
        g)
                use_gpu=$OPTARG
        ;;
...
if [[ "$restraints_pickle" == "" ]] ; then
    restraints_pickle="None"
fi

if [[ "$feature_pickle" == "" ]] ; then
    feature_pickle="None"
fi
if [[ "$use_gpu" == "" ]] ; then
    use_gpu=true
...
alphafold_script="$current_working_dir/run_grasp_with_em.py"
...
command_args="--fasta_paths=$fasta_path --output_dir=$output_dir --feature_pickle=$feature_pickle --restraints_pickle=$restraints_pickle --max_template_date=$max_template_date --db_preset=$db_preset --model_preset=$model_preset --benchmark=$benchmark --use_precomputed_msas=$use_precomputed_msas --num_multimer_predictions_per_model=$num_multimer_predictions_per_model --models_to_relax=$run_relax --use_gpu_relax=$use_gpu_relax --mrc_path=$mrc_path --resolution=$resolution --name=$name"
